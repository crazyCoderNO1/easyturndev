...
+ (void)setConstraintSize:(CGSize)constraintSize {
    [self sharedView].constraintSize = constraintSize;
}
...
+ (void)setViewForHUDView:(UIView*)view {
    [self sharedView].loadingView = view;
}
...
+ (void)showViewHUD:(UIView *)view status:(NSString*)status {
    [[self sharedView] showViewHUD:view status:status];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _constraintSize = CGSizeMake(200.0f, 300.0f);
...
- (void)updateHUDFrame {
    ...
    BOOL imageUsed = (self.imageView.image) && !(CGSizeEqualToSize(self.imageView.image.size, CGSizeZero)) && !(self.imageView.hidden);
    ...
    if(self.statusLabel.text) {
        CGSize constraintSize = self.constraintSize;
    ...
    self.loadingView.center = CGPointMake(CGRectGetMidX(self.hudView.bounds), centerY);
...
- (void)showViewHUD:(UIView *)view status: (NSString *)status {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Update loadingView
            strongSelf.loadingView.tintColor = strongSelf.foregroundColorForStyle;
            strongSelf.loadingView = view;
            strongSelf.loadingView.hidden = NO;
            
            if (_loadingView.superview == nil) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
                [strongSelf.hudView.contentView addSubview:strongSelf.loadingView];
#else
                [strongSelf.hudView addSubview:strongSelf.loadingView];
#endif
            }
...
- (UIView *)loadingView {
    if (_loadingView == nil) {
        ...
        CGFloat radius  = self.statusLabel.text ? self.ringRadius : self.ringNoTextRadius;;
...
- (void)setConstraintSize:(CGSize)constraintSize {
    if (!_isInitializing) _constraintSize = constraintSize;
}
